...
$stmt = $db->query('SELECT * FROM teachers WHERE id = :receiver_id', [':receiver_id' => $receiver_id]);

$stmt2 = $db->query('SELECT * FROM messages WHERE sender_id = :sender_id AND receiver_id = :receiver_id OR receiver_id = :sender_id and sender_id = :receiver_id ORDER BY id ASC;', [':receiver_id' => $receiver_id, ':sender_id' => $sender_id]);
